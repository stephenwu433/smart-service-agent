...
| 4 | 我之前说错了，我接的是 C2，不是 C1 | 收到，已更正为 C2。刚才的插座测试先不用做，因为 A1289 自身充电需要使用 C1。请改接 C1，其他条件保持不变，再观察屏幕是否出现输入功率。 | GUIDE | `confirmation_type=fact_correction`，撤回依赖 C1 的 Attempt，新增依赖 C2 的 Attempt |
...
1. **事实更正直接接受**（不做二次确认）。2 号明确说"我接的是 C2，不是 C1"时，系统直接
   更新 Case revision，撤回依赖旧值的 Attempt，给出改接 C1 的指引。
2. **`safety_precheck` 和 `resolved_check` 阻塞**，需要用户回复；`fact_correction` 不阻塞。
