...
| 4 | 我之前说错了，我接的是 C2，不是 C1 | 收到，已更正为 C2。刚才的插座测试先不用做，因为 A1289 自身充电需要使用 C1。请改接 C1，其他条件保持不变，再观察屏幕是否出现输入功率。 | GUIDE | 直接写入 revision，撤回依赖 C1 的 Attempt，新增依赖 C2 的 Attempt |
...
1. **明确事实更正直接生效**。用户明确说「我之前说错了，我接的是 C2，不是 C1」时，系统立即
   更新 Case revision、撤回依赖旧值的 Attempt，并给出改接 C1 的指引；只有“可能、好像、记不清”
   等模糊表达才返回 `ASK`。
2. **`safety_precheck` 和 `resolved_check` 会阻塞推进**，需要用户回复；已应用的
   `fact_correction` 不阻塞。
