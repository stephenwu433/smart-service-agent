...
- 跨轮明确更正：用户表达“说错 / 实际是 / 其实”等触发词并给出明确新值时，系统直接更新
  Case revision，撤回依赖旧值的 Attempt，写入 `case_revised` 审计，并返回受影响步骤和下一步；
  不要求消费者再回复一次“确认”。
- 模糊更正：含“可能、好像、也许、不确定、记不清”等措辞时返回 `state=ASK`，确认实际事实后
  才能继续。
...
- 用户在 GUIDE 后回复“已恢复、部分改善、仍无改善、无法执行或跳过”时，系统自动更新最近一条
  有效 Attempt 的 `execution_status`、`observation` 和 `outcome`。
- 用户首次报告恢复时记录 `outcome=improved` 并继续观察；稳定性确认后改为
  `outcome=resolved`。
- 自动 GUIDE 会复用相同的有效未执行 Attempt，不重复建步骤。
